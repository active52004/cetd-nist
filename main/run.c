...
//CETD API
#include "./cetd/include/config.h"
#include "./cetd/include/show.h"
#include "./cetd/include/input.h"
#include "./cetd/include/cipher.h"
#include "./cetd/include/write_txt.h"
#include "./cetd/include/write_csv.h"
#include "./cetd/include/math_func.h"
#include "./cetd/include/CETD.h"
#include "./cetd/include/aes.h"
#include "./cetd/include/LL_arr_gen.h"
...
/*
argv[]={shuffle_round, num-gf-mult, seq-len, input-len } 
 */
...
//	file_type = displayFileFormat();
	...
	int block_number;//No. of blks per plaintext
	int block_length; //No. of byte per plaintext
	int CETD_tag_length; //No. of bytes per tag
	int test_count;//No. of bit sequences
    int n;//number of tags per bit sequence

	int shuffle_round; //No. of shuffle round
//	printf("Input No. of shuffle rounds:\n");
//	scanf("%d",&shuffle_round);
	shuffle_round = atof(argv[1]);//first argv para

	int num_blk_mult;//No. of input blks doing gf-mult
//	printf("Input No. of GF-mult input blocks:\n");
//	scanf("%d", &num_blk_mult);
	num_blk_mult = atof(argv[2]);//second argv para
...
//    	scanf("%d",&n);
		n = atof(argv[3]); //sequence length, third argv para
//		printf("n is %d\n",n);
...
//		scanf("%d", &block_length);
		block_length = atof(argv[4]); //No. of bytes per input, fourth argv para
